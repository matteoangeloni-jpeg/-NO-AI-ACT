 ...
 * la piramide del rischio, poi la misura correttiva. I reperti citati in
 * EvidenceScene concorrono alla valutazione.
 */
export class DecisionScene extends Phaser.Scene {
  ...
  private header(step: string, question: string): void {
    ...
    const texts = L();
    this.add
      .text(cx, 56, `${fmt(texts.ui.case.fileLabel, { code: this.caseData.fileCode })}`, textStyle(13, COLOR_STR.alertText))
      .setOrigin(0.5);
  ...
  private showClassificationStep(): void {
    ...
    this.add
      .text(cx, 170, L().ui.decision.contextNote, textStyle(12, COLOR_STR.paperDim, { wordWrap: { width: 880 }, align: 'center' }))
      .setOrigin(0.5, 0);
    ...
    CLASSIFICATIONS.forEach((cls, i) => {
      new Button(this, cx, 230 + i * 64, `${i + 1}. ${L().classifications[cls].toUpperCase()}`, () => pick(cls), { width: 460 });
    });
    this.bindNumberKeys(CLASSIFICATIONS.length, (i) => pick(CLASSIFICATIONS[i]));
    this.add
      .text(cx, GAME_HEIGHT - 80, L().ui.decision.keys5, textStyle(12, COLOR_STR.paperDim))
      .setOrigin(0.5);
  ...
  private showMeasureStep(): void {
    ...
    MEASURES.forEach((measure, i) => {
      ...
      new Button(this, i === MEASURES.length - 1 && MEASURES.length % 2 === 1 ? cx : x, 230 + row * 64, `${i + 1}. ${L().measures[measure].toUpperCase()}`, () => this.resolve(measure), { width: 440, fontSize: 14 });
    });
    this.bindNumberKeys(MEASURES.length, (i) => this.resolve(MEASURES[i]));
    this.add
      .text(cx, GAME_HEIGHT - 80, L().ui.decision.keys7, textStyle(12, COLOR_STR.paperDim))
      .setOrigin(0.5);
  }
  ...
  private resolve(measure: Measure): void {
    ...
    this.resolved = true;
    const quality = evaluateDecision(this.caseData, this.classification!, measure, this.citedClues);
    const cluesOk = cluesSupportClassification(this.caseData, this.citedClues);
